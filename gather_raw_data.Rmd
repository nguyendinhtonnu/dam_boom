...
import_dams <- function(file, region){
 ...
 # Import GRandD data for each world region using import_dams()
...
oceania <- import_dams("raw_data/dams/oceania_aquastat.xlsx", "Oceania")
  ...
  # Import 2 datasets implified for about page
...
# Create function for importing wb data from wbstats package. 
...
# subnational level population data
...
# gridded population, gdp, ppp data from g-econ

gridded_data <- read_excel("raw_data/subnational/spatialecon-gecon-v4-xls/Original data/gecon-v4.xls", 
    sheet = "Sheet1") %>%
  clean_names()

saveRDS(gridded_data, "shiny_app/gridded_data")

```
```{r file_path}

# create file path for all shape files

file_path <- tibble(country_name = c("Argentina", "Bolivia (Plurinational State of)", "Brazil", 
                                     "Burkina Faso", "China", "Cuba", "India", "Indonesia", 
                                "Iran (Islamic Republic of)", "Lao People's Democratic Republic", "Mexico", 
                                "Romania", "Russian Federation","South Africa", "Turkey", "Uruguay", 
                                    "Zimbabwe", "World Basins"),
                   ...
                   "raw_data/basins/Major_Basins_of_the_World.shp"
                   ))
...
joined <- all_dams %>%
  group_by(code, year) %>%
  mutate(dams_count = n()) %>%
  select(code, year, dams_count, country) 
